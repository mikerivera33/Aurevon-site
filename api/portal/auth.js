...
export default async function handler(req, res) {
  ...
  const BASE_URL = process.env.BASE_URL || 'https://blockt.co';
  ...
  // Search Leads table for customer email
  ...
  try {
    ...
    const magicLink = `${BASE_URL}/portal.html?token=${token}&email=${encodeURIComponent(normalizedEmail)}`;

    const emailHtml = `<!DOCTYPE html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<style>
  body{margin:0;padding:0;background:#0A0A0A;font-family:'DM Sans',Arial,sans-serif;color:#f0ede8;}
  .wrap{max-width:560px;margin:40px auto;background:#111827;border-radius:16px;overflow:hidden;}
  .header{background:linear-gradient(135deg,#1E3A8A,#3B82F6);padding:32px 40px;text-align:center;}
  .header h1{margin:0;font-size:28px;font-weight:900;color:#fff;letter-spacing:-0.5px;}
  .body{padding:40px;}
  .greeting{font-size:18px;color:#f0ede8;margin-bottom:16px;}
  .msg{font-size:15px;color:#b8b3ab;line-height:1.6;margin-bottom:32px;}
  .btn{display:block;background:linear-gradient(135deg,#1E3A8A,#3B82F6);color:#fff;text-decoration:none;text-align:center;padding:18px 32px;border-radius:10px;font-size:17px;font-weight:700;letter-spacing:0.3px;margin-bottom:24px;}
  .note{font-size:13px;color:#6b6760;text-align:center;line-height:1.5;}
  .footer{border-top:1px solid #2a2a2e;padding:24px 40px;text-align:center;}
  .footer p{font-size:12px;color:#6b6760;margin:0;}
</style></head><body>
<div class="wrap">
  <div class="header"><h1>Aurevon</h1></div>
  <div class="body">
    <p class="greeting">${greeting}</p>
    <p class="msg">Click the button below to access your Aurevon Dashboard. Your deal statuses, NFTs, and purchase history are all inside.</p>
    <a href="${magicLink}" class="btn">Open My Dashboard →</a>
    <p class="note">This link expires in 15 minutes. If you didn't request this, you can safely ignore this email.</p>
  </div>
  <div class="footer"><p>Aurevon Group LLC &nbsp;·&nbsp; <a href="mailto:hello@aurevongroup.com" style="color:#6b6760;">hello@aurevongroup.com</a></p></div>
</div>
</body></html>`;
