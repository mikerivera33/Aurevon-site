...
async function handleAuth(req, res) {
  ...
  try {
      ...
      const existing = await fetchRecords(AUTH_TABLE, `LOWER({Email})="${normalizedEmail}"`);
        if (existing.length > 0) {
                const expiry = existing[0].fields['Token Expires'];
      ...
      const token = crypto.randomBytes(32).toString('hex');
        ...
        await upsertAuthRecord(normalizedEmail, { 'Magic Token': token, 'Token Expires': expiryTs, 'Session Active': false });
...
async function handleVerify(req, res) {
  ...
  try {
      ...
      const record = records[0];
        ...
        const storedToken = fields['Magic Token'] ?? '';
      ...
      if (storedToken.length !== token.length) return res.status(401).json({ error: INVALID_MSG });
        ...
        if (!fields['Token Expires'] || new Date(fields['Token Expires']) <= new Date()) {
                return res.status(401).json({ error: INVALID_MSG });
        }
      ...
      const sessionToken = `${Date.now().toString(36)}.${crypto.randomBytes(32).toString('hex')}`;
        ...
        const updateResp = await fetch(url, {
                ...
                body: JSON.stringify({ fields: { 'Magic Token': '', 'Session Active': true, 'Session Token': sessionToken } }),
        });
        if (!updateResp.ok) {
